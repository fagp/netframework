...
class LandscapeVis():
    def __init__(self, defaults_path):
        ...
        parser.add_argument('--image_num', nargs='?', type=int, default=-1, help='Select image')
        ...
        if os.path.isfile(folders['plane_path_x']):
            self.wx=self.loadmodel(self.net,folders['plane_path_x'])
        else:
            states = self.net.state_dict()
            self.wx=get_random_states(states)
            normalize_directions_for_states(self.wx, self.net.state_dict(), args.xnorm, args.xignore)
            self.savemodel(self.net,self.wx,folders['plane_path_x'])

        if os.path.isfile(folders['plane_path_y']):
            self.wy=self.loadmodel(self.net,folders['plane_path_y'])
        else:
            states = self.net.state_dict()
            self.wy=get_random_states(states)
            normalize_directions_for_states(self.wy, self.net.state_dict(), args.ynorm, args.yignore)
            self.savemodel(self.net,self.wy,folders['plane_path_y'])
    ...
    def validation_im(self,cx,cy,i): 
        ttime=time.time()  
        
        print('Computing {} {}:'.format(cx,cy), end=' ')
        lendata=len(self.traindataset)
        with torch.no_grad():    
            sample=self.traindataset[i % lendata]
    ...
    def loadmodel(self,net,modelpath):
        if os.path.isfile(modelpath):
            ...
            to_load= netx.module if self.use_parallel else netx
            if 'weights' in checkpoint.keys():
                return checkpoint['weights']
            to_load.load_state_dict(checkpoint['net'])

            return create_target_direction(net,netx)
        else:
            raise Exception('Model not found')

    def savemodel(self,net,weights,modelpath=''):
        if modelpath=='':
